		...
		<tr>
			...
			<td rowspan="5">
				<table style="width: 100%">
					...
					<c:forEach var="role" items="${rolelst}">
						<tr>
							<td>${role.roleId}</td>
							<td>${role.roleName}</td>
					<form method="post" action="${pageContext.request.contextPath}/deleterole">
							<input type="hidden" name="id" value="${role.roleId}"  />
							<td><input type="submit" value="Delete"  /></td>
						</form>
						
						<form method="get" action="${pageContext.request.contextPath}/updaterole">
							<input type="hidden" name="id" value="${role.roleId}"  />
